...
void fwu_img_parse_format(void);
...
static int fwu_read_f34_queries(void)
{
	...
	if (fwu->flash_properties.has_flash_query4) {
		...
		retval = fwu->fn_ptr->read(rmi4_data,
			fwu->f34_fd.query_base_addr + fwu->blk_size_off + 2,
			query4.data,
			sizeof(query4.data));
		if (retval < 0) {
			dev_err(&rmi4_data->i2c_client->dev,
		...
		if (query4.has_guest_code) {
			retval = fwu->fn_ptr->read(rmi4_data,
				fwu->f34_fd.query_base_addr + fwu->blk_size_off + 3,
				buf,
				2);
			if (retval < 0) {
				dev_err(&rmi4_data->i2c_client->dev,
			...
			dev_info(&rmi4_data->i2c_client->dev,
					"%s: query data do not supply guest image.\n",
					__func__);
...
static int fwu_do_reflash(void)
{
	...
	if (fwu->img.uiFirmware.data) {
		...
		if (retval < 0)
			return retval;
		dev_info(&rmi4_data->i2c_client->dev, "%s: Firmware programmed\n",
				__func__);
	}
	...
	if (fwu->img.uiConfig.data) {
		...
		if (retval < 0)
			return retval;
		dev_info(&rmi4_data->i2c_client->dev, "%s: Configuration programmed\n",
				__func__);
	}
	...
	if (fwu->img.guestCode.data) {
		...
		if (retval < 0)
			return retval;
		dev_info(&rmi4_data->i2c_client->dev, "%s: guest-thread programmed\n",
				__func__);
	} else {
		dev_info(&rmi4_data->i2c_client->dev, "%s: is not guestcode data in fw img\n",
				__func__);
...
write_config:
	...
	dev_info(&rmi4_data->i2c_client->dev, "%s: Config written\n",
			__func__);
...
static int fwu_start_reflash(void)
{
	...
	while (retry--) {
		...
		case NONE:
		default:
			dev_info(&rmi4_data->i2c_client->dev, "%s: case is NONE or default\n",
					__func__);
			goto exit;
	}
...
exit:
	...
	dev_info(&rmi4_data->i2c_client->dev, "%s: End of reflash process\n",
			__func__);
...
static int fwu_do_write_guest_code(void)
{
	...
	dev_info(&fwu->rmi4_data->i2c_client->dev, "%s: guest code written\n",
			__func__);
...
static int fwu_start_write_config(void)
{
	...
	dev_info(&rmi4_data->i2c_client->dev, "%s: Start of write config process\n",
			__func__);
	...
	dev_info(&rmi4_data->i2c_client->dev, "%s: End of write config process\n",
			__func__);
...
exit:
	fwu->rmi4_data->reset_device(rmi4_data);
...
void fwu_img_scan_x10_container(unsigned int listLength, unsigned char *startAddr)
{
	unsigned int i;
	unsigned int length;
...
void fwu_img_parse_x10_topcontainer(void)
{
...
void fwu_img_parse_x10(void)
{
	fwu_img_parse_x10_topcontainer();
}

void fwu_img_parse_x0_x6(void)
{
